...
/* ================= ASSEMBLY ================= */
(() => {
  const svg = $('assembly'); if (!svg) return;
  const CELLS = [
    ['M0,0 A1,1 0 0,1 1,1 L0,1 Z',      [0,0,1,1],       [-1.8,-1.4,-1.8,0]],
    ['M0,1 L1,1 L0,2 Z',                [0,1.5,1,1.5],   [-2.6,0,null,null]],
    ['M1,1 L1,2 L0,2 Z',                [0,2,1,1],       [-1.4,1.8,-1.4,0]],
    ['M0,2 A1,1 0 0,0 1,3 L1,2 Z',      [0.5,2,1,3],     [0,2.1,null,null]],
    ['M1,1 A1,1 0 0,1 2,0 L2,1 Z',      [1,0.5,2,1],     [1.5,-1.9,0,-1.9]],
    ['M2,0 A1,1 0 0,1 3,1 L2,1 Z',      [2,0,3,1],       [0,-2.2,null,null]],
    ['M1,1 A1,1 0 0,0 2,2 L2,1 Z',      [1,1,2,2],       [2.2,-1.1,2.2,0]],
    ['M2,1 A1,1 0 0,1 3,2 L2,2 Z',      [2.5,1,2.5,2],   [2.6,0,null,null]],
    ['M3,2 A1,1 0 0,1 2,3 L2,2 Z',      [2,2,3,3],       [1.6,1.6,0,1.6]],
    ['M2,3 A1,1 0 0,1 1,2 L2,2 Z',      [1,2.6,2,2.6],   [0,2.4,null,null]],
  ];
  svg.innerHTML = '<defs>' + CELLS.map((c, i) => {
    const [x1, y1, x2, y2] = c[1];
    return `<linearGradient id="ag${i}" gradientUnits="userSpaceOnUse" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}"><stop offset="0"/><stop offset="1"/></linearGradient>`;
  }).join('') + '</defs>' + CELLS.map((c, i) => `<g><path d="${c[0]}" fill="url(#ag${i})"/></g>`).join('');
  const groups = [...svg.querySelectorAll('g')];

  let randFams = null; // array of per-cell family indices (0-4), or null = signature colourway
  function colors() {
    CELLS.forEach((c, i) => {
      const [a, b] = randFams ? P().fams[randFams[i]] : P().asm[i];
      const g = svg.querySelector(`#ag${i}`);
      if (!g) return;
      g.children[0].setAttribute('stop-color', a);
      g.children[1].setAttribute('stop-color', b);
    });
  }
  colors(); REPAINT.push(colors);

  const LEG = 'cubic-bezier(0.6, 0, 0.12, 1)';
  function play() {
    svg.getAnimations({ subtree:true }).forEach(a => a.cancel());
    if (REDUCED) { groups.forEach(g => g.style.opacity = 1); return; }
    let total = 0;
    groups.forEach((g, i) => {
      const [sx, sy, cx, cy] = CELLS[i][2];
      const delay = i * 80, dur = 950;
      total = Math.max(total, delay + dur);
      const frames = (cx === null)
        ? [{ transform:`translate(${sx}px, ${sy}px) rotate(-90deg)`, opacity:0, easing:LEG },
           { opacity:1, offset:0.12 },
           { transform:'translate(0,0) rotate(0deg)', opacity:1 }]
        : [{ transform:`translate(${sx}px, ${sy}px) rotate(-180deg)`, opacity:0, easing:LEG },
           { opacity:1, offset:0.10 },
           { transform:`translate(${cx}px, ${cy}px) rotate(-90deg)`, opacity:1, offset:0.52, easing:LEG },
           { transform:'translate(0,0) rotate(0deg)', opacity:1 }];
      g.animate(frames, { duration:dur, delay, easing:'linear', fill:'both' });
    });
    svg.animate([{ transform:'scale(1)' }, { transform:'scale(1.014)' }, { transform:'scale(1)' }],
      { duration:240, delay:total + 40, easing:'ease-out' });
  }
  $('replay').addEventListener('click', play);
  mkRand($('replay'), () => { randFams = CELLS.map(() => randInt(5)); colors(); play(); },
    { cls:'btn-quiet', label:'Randomise colours', aria:'Recolour the monogram and replay' });
  if (REDUCED) play();
  else {
